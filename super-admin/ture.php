...
$sql = "SELECT * FROM user where userlevel = 3 and business_id = " . $_GET['bid'];
$res = mysqli_query($conn, $sql);
$drivers = array();
if ($res) {
  while ($row = mysqli_fetch_assoc($res)) {
...
if (count($_POST) > 0) {
  if ($method == 'add') {
    ...
    if ($do_insert) {
      custom_redirect("Location: " . HOME_URL . "/super-admin/ture.php?success=1&bid=" . $_GET['bid']);
      die();
    } else {
      custom_redirect("Location: " . HOME_URL . "/super-admin/ture.php?error=1&task=add&bid=" . $_GET['bid']);
      die();
    }
  }
  ...
  if ($method == 'edit') {
    ...
    if ($do_insert) {
      custom_redirect("Location: " . HOME_URL . "/super-admin/ture.php?action=edit&method=edit&id=" . $tripid . "&success=1&bid=" . $_GET['bid']);
      die();
    } else {
      custom_redirect("Location: " . HOME_URL . "/super-admin/ture.php?error=1&task=add&bid=" . $_GET['bid']);
      die();
...
if ($method == 'delete') {
  ...
  if ($do_del) {
    custom_redirect("Location: super-admin/ture.php?bid=" . $_GET['bid']);
  } else {
    custom_redirect("Location: super-admin/ture.php?error=1&task=delete&bid=" . $_GET['bid']);
  }
  die();
}
if ($method == 'edit') {
    ...
    <div class="content-wrapper">
      ...
      <?php if ($method == 'index') : ?>
        <section class="content">
          ...
          <!-- /.box -->
          <div>
            <a href="<?php echo HOME_URL; ?>/super-admin/ture.php?method=add&bid=<?php echo $_GET['bid']; ?>" class="btn btn-primary">Tilføj Ny</a>
          </div>
          <!-- /.col -->
          <!-- /.row -->
        </section>
          ...
          <form class="form-horizontal" action="" method="POST">
            ...
            <div class="form-group">
              <div class="col-sm-offset-2 col-sm-10">
                <a href="<?php echo HOME_URL; ?>/super-admin/ture.php?bid=<?php echo $_GET['bid']; ?>" class="btn btn-danger">Annuller</a>
